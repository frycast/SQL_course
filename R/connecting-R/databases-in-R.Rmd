...
Setting up and connecting to database servers can get tricky, with all the authentication methods and security measures, as well as hardware differences, port configurations, driver requirements, and so on. This goes to show that much of SQL is still the domain of engineers and IT professionals.
 
With this guide, I'm somewhat ambitiously aiming to make it relatively easy for a newcomer (with basic R knowledge) to do all of the following:

1. Create and connect to a local server.
2. Connect to a remote database.
3. Create and use a (serverless) SQLite database.

We'll learn how to do numbers 1 and 2 in both T-SQL and MySQL. 

Number 3, SQLite, is a lightweight dialect of SQL that is a great way to, for example, store multiple CSVs in one single file (as separate SQL tables), that can then be queried using SQL code, without needing a server! SQLite is great, because it's a full-featured, highly reliable database engine and, being a single file (and serverless), the database is really easy to set up and share.

## 1. Create and connect to a local server

Before connecting to a local database server in R, you should create the server, install a popular SQL code editor, and insert some practice data. For this, I've created the two guides linked below. Please go through one of them before continuing.
...
Connecting to a remote database means you can avoid the hassle of installing a local MySQL or T-SQL server. You will need some connection details specific to the database you plan to connect to.
